...
if len(sys.argv) <= 1:
    ...
    for opt, arg in opts:
        if opt in ('--config'):
            with open(os.path.join(r"..", "configs", arg + '.json'),
                      mode='r') as fid:
                data = json.load(fid)
...
row_radius = (
    (mount_height + extra_height) / 2) / (np.sin(alpha / 2)) + cap_top_height
column_radius = (((mount_width + extra_width) / 2) /
                 (np.sin(beta / 2))) + cap_top_height
...
def single_plate(cylinder_segments=100, side="right"):
    ...
    if plate_style in ['NUB', 'HS_NUB']:
        ...
        top_wall = translate(top_wall,
                             (0, (tb_border / 2) +
                              (keyswitch_height / 2), plate_thickness / 2))

        lr_border = (mount_width - keyswitch_width) / 2
        left_wall = box(lr_border, mount_height, plate_thickness)
        left_wall = translate(left_wall,
                              ((lr_border / 2) +
                               (keyswitch_width / 2), 0, plate_thickness / 2))
        ...
        shape_cut = box(keyswitch_width, keyswitch_height,
                        mount_thickness * 2 + .02)
    ...
    if plate_style in ['UNDERCUT', 'HS_UNDERCUT', 'NOTCH', 'HS_NOTCH']:
        ...
        if plate_style in ['NOTCH', 'HS_NOTCH']:
            undercut = box(notch_width, keyswitch_height + 2 * clip_undercut,
                           mount_thickness)
            undercut = union([
                undercut,
                box(keyswitch_width + 2 * clip_undercut, notch_width,
                    mount_thickness)
            ])
        ...
        if ENGINE == 'cadquery' and undercut_transition > 0:
            undercut = undercut.faces("+Z").chamfer(undercut_transition,
                                                    clip_undercut)
    ...
    if plate_holes:
        ...
        holes = [
            translate(
                cylinder(radius=plate_holes_diameter / 2,
                         height=plate_holes_depth + .01),
                (x_off + half_width, y_off + half_height,
                 plate_holes_depth / 2 - .01)),
            translate(
                cylinder(radius=plate_holes_diameter / 2,
                         height=plate_holes_depth + .01),
                (x_off - half_width, y_off + half_height,
...
def trackball_cutout(segments=100, side="right"):
    if trackball_modular:
        hole_diameter = ball_diameter + 2 * (ball_gap + ball_wall_thickness +
                                             trackball_modular_clearance +
                                             trackball_modular_lip_width) - .1
...
def sa_cap(Usize=1):
    ...
    k1 = polyline([(bw2, bl2), (bw2, -bl2), (-bw2, -bl2), (-bw2, bl2),
                   (bw2, bl2)])
    k1 = extrude_poly(outer_poly=k1, height=0.1)
    k1 = translate(k1, (0, 0, 0.05))
    k2 = polyline([(pw2, pl2), (pw2, -pl2), (-pw2, -pl2), (-pw2, pl2),
                   (pw2, pl2)])
...
def key_pcb():
    ...
    holes = [
        translate(
            hole,
            (pcb_hole_pattern_width / 2, pcb_hole_pattern_height / 2, 0)),
        translate(
            hole,
            (-pcb_hole_pattern_width / 2, pcb_hole_pattern_height / 2, 0)),
        translate(
...
def plate_pcb_cutouts(side="right"):
    ...
    for column in range(ncols):
        for row in range(nrows):
            if (column in [2, 3]) or (not row == lastrow):
                cutouts.append(
                    key_place(plate_pcb_cutout(side=side), column, row))
...
def web_post_tr(wide=False):
    ...
    return translate(web_post(), ((mount_width / w_divide) - post_adj,
                                  (mount_height / 2) - post_adj, 0))
...
def thumborigin():
    ...
    origin = key_position([mount_width / 2, -(mount_height / 2), 0], 1,
                          cornerrow)
    ...
    if cap:
        shape_list = [
            default_thumb_mr_place(
                rotate(shape, [0, 0, thumb_plate_mr_rotation])),
            default_thumb_ml_place(
                rotate(shape, [0, 0, thumb_plate_ml_rotation])),
            default_thumb_br_place(
                rotate(shape, [0, 0, thumb_plate_br_rotation])),
            default_thumb_bl_place(
                rotate(shape, [0, 0, thumb_plate_bl_rotation])),
        ]

        if default_1U_cluster:
            shape_list.append(
                default_thumb_tr_place(
                    rotate(rotate(shape, (0, 0, 90)),
                           [0, 0, thumb_plate_tr_rotation])))
            shape_list.append(
                default_thumb_tr_place(
                    rotate(rotate(shape, (0, 0, 90)),
                           [0, 0, thumb_plate_tr_rotation])))
            shape_list.append(
                default_thumb_tl_place(
                    rotate(shape, [0, 0, thumb_plate_tl_rotation])))
...
def default_thumb_pcb_plate_cutouts(side="right"):
    ...
    shape = union(
        [shape, default_thumb_15x_layout(plate_pcb_cutout(side=side))])
    return shape
...
def default_thumb_15x_layout(shape, cap=False, plate=True):
    ...
    if plate:
        if cap:
            ...
            cap_list.append(
                default_thumb_tr_place(
                    rotate(shape, [0, 0, thumb_plate_tr_rotation])))
            return add(cap_list)
        else:
            shape_list = [
                default_thumb_tl_place(
...
def adjustable_plate_half(Usize=1.5):
    ...
    top_plate = translate(top_plate, [
        0, (adjustable_plate_height + mount_height) / 2, plate_thickness -
        (web_thickness / 2)
    ])
    return top_plate
...
def adjustable_square_plate(Uwidth=1.5, Uheight=1.5):
    ...
    print("width: {}, height: {}, thickness:{}".format(width, height,
                                                       web_thickness))
    shape = box(width, height, web_thickness)
    shape = difference(
        shape, [box(mount_width - .01, mount_height - .01, 2 * web_thickness)])
...
def double_plate_half():
    ...
    top_plate = translate(top_plate, [
        0, (double_plate_height + mount_height) / 2, plate_thickness -
        (web_thickness / 2)
    ])
    return top_plate
...
def default_thumb(side="right"):
    print('thumb()')
    shape = default_thumb_1x_layout(
        rotate(single_plate(side=side), (0, 0, -90)))
    shape = union([
        shape,
        default_thumb_15x_layout(rotate(single_plate(side=side), (0, 0, -90)))
    ])
    shape = union(
        [shape, default_thumb_15x_layout(double_plate(), plate=False)])
...
def thumb_post_tr():
    ...
    return translate(
        web_post(), [(mount_width / 2) - post_adj,
                     ((mount_height / 2) + double_plate_height) - post_adj, 0])
...
def mini_thumb_15x_layout(shape):
    return union(
        [mini_thumb_tr_place(rotate(shape, [0, 0, thumb_plate_tr_rotation]))])
...
def mini_thumb_post_tr():
    return translate(web_post(), [(mount_width / 2) - post_adj,
                                  (mount_height / 2) - post_adj, 0])
...
def minidox_thumb(side="right"):
    ...
    shape = minidox_thumb_fx_layout(
        rotate(single_plate(side=side), [0.0, 0.0, -90]))
    shape = union(
        [shape,
         minidox_thumb_fx_layout(adjustable_plate(minidox_Usize))])
...
def minidox_thumb_post_tr():
    ...
    return translate(web_post(), [
        (mount_width / 2) - post_adj,
        ((mount_height / 2) + adjustable_plate_size(minidox_Usize)) - post_adj,
        0
    ])
...
def minidox_thumb_post_bl():
    ...
    return translate(web_post(), [
        -(mount_width / 2) + post_adj,
        -((mount_height / 2) + adjustable_plate_size(minidox_Usize)) +
        post_adj, 0
    ])
...
def carbonfet_thumb_1x_layout(shape):
    return union([
        #return add([
        carbonfet_thumb_tr_place(rotate(shape,
                                        [0, 0, thumb_plate_tr_rotation])),
        carbonfet_thumb_mr_place(rotate(shape,
                                        [0, 0, thumb_plate_mr_rotation])),
        carbonfet_thumb_br_place(rotate(shape,
                                        [0, 0, thumb_plate_br_rotation])),
        carbonfet_thumb_tl_place(rotate(shape,
                                        [0, 0, thumb_plate_tl_rotation])),
    ])
...
def carbonfet_thumb_15x_layout(shape, plate=True):
    if plate:
        return union([
            #return add([
            carbonfet_thumb_bl_place(
                rotate(shape, [0, 0, thumb_plate_bl_rotation])),
            carbonfet_thumb_ml_place(
                rotate(shape, [0, 0, thumb_plate_ml_rotation]))
        ])
    else:
        return union([
...
def carbonfet_thumbcaps():
    ...
    t15 = carbonfet_thumb_15x_layout(
        rotate(sa_cap(1.5), [0, 0, rad2deg(pi / 2)]))
    return t1.add(t15)
...
def carbonfet_thumb(side="right"):
    ...
    shape = union(
        [shape,
         carbonfet_thumb_15x_layout(double_plate_half(), plate=False)])
...
def tbjs_thumb_position_rotation():
    ...
    shift = [
        -.9 * tbjs_key_diameter / 2 + 27 - 42,
        -.1 * tbjs_key_diameter / 2 + 3 - 25, -5
    ]
...
def tbjs_thumb_tl_place(shape):
    ...
    shape = translate(shape,
                      (t_off[0], t_off[1] + tbjs_key_diameter / 2, t_off[2]))
...
def tbjs_thumb(side="right"):
    ...
    shape = union([
        shape, *tbjs_thumb_fx_layout(
            adjustable_square_plate(Uwidth=tbjs_Uwidth, Uheight=tbjs_Uheight))
    ])
...
def tbjs_thumb_post_tr():
    ...
    return translate(
        ...
        [(mount_width / 2) + adjustable_plate_size(tbjs_Uwidth) - post_adj,
         ((mount_height / 2) + adjustable_plate_size(tbjs_Uheight)) - post_adj,
         0])
...
def tbjs_post_l():
    ...
    radius = ball_diameter / 2 + ball_wall_thickness + ball_gap
    return translate(
        web_post(), [-1.0 * (radius - post_adj), 0.0 * (radius - post_adj), 0])
...
def tbcj_holder():
    ...
    shape = difference(
        shape, [cylinder(tbcj_inner_diameter / 2, tbcj_thickness + 0.1)])
...
def left_key_position(row, direction, low_corner=False, side='right'):
    ...
    pos = np.array(
        key_position([-mount_width * 0.5, direction * mount_height * 0.5, 0],
                     0, row))
    if trackball_in_wall and (side == ball_side or ball_side == 'both'):
        ...
        return list(pos - np.array([
            ...
            x_offset, -y_offset, tbiw_left_wall_z_offset_override + z_offset
        ]))
    ...
    return list(pos - np.array([
        left_wall_x_offset - x_offset, -y_offset, left_wall_z_offset + z_offset
    ]))
...
def back_wall(skeleton=False):
    ...
    if not skeleton:
        shape = union([
            shape,
            key_wall_brace(lastcol, 0, 0, 1, web_post_tr(), lastcol, 0, 1, 0,
                           web_post_tr())
        ])
    return shape
...
def front_wall(skeleton=False):
    ...
    shape = union([
        shape,
        key_wall_brace(3, lastrow, 0, -1, web_post_bl(), 3, lastrow, 0.5, -1,
                       web_post_br())
    ])
    shape = union([
        shape,
        key_wall_brace(3, lastrow, 0.5, -1, web_post_br(), 4, cornerrow, 1, -1,
                       web_post_bl())
    ])
    for i in range(ncols - 4):
        ...
        shape = union([
            shape,
            key_wall_brace(x, cornerrow, 0, -1, web_post_bl(), x, cornerrow, 0,
                           -1, web_post_br())
        ])
    for i in range(ncols - 5):
        ...
        shape = union([
            shape,
            key_wall_brace(x, cornerrow, 0, -1, web_post_bl(), x - 1,
                           cornerrow, 0, -1, web_post_br())
        ])
...
def thumb_walls(side='right', style_override=None, skeleton=False):
    ...
    if _thumb_style == "MINI":
        ...
        if (side == ball_side or ball_side == 'both'):
            ...
            return thumb_walls(side,
                               style_override=other_thumb,
                               skeleton=skeleton)
    else:
...
def default_thumb_walls(skeleton=False):
    ...
    if default_1U_cluster:
        shape = union([
            wall_brace(default_thumb_mr_place, 0, -1, web_post_br(),
                       default_thumb_tr_place, 0, -1, web_post_br())
        ])
    else:
        shape = union([
            wall_brace(default_thumb_mr_place, 0, -1, web_post_br(),
                       default_thumb_tr_place, 0, -1, thumb_post_br())
        ])
    shape = union([
        shape,
        wall_brace(default_thumb_mr_place, 0, -1, web_post_br(),
                   default_thumb_mr_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_br_place, 0, -1, web_post_br(),
                   default_thumb_br_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_ml_place, -0.3, 1, web_post_tr(),
                   default_thumb_ml_place, 0, 1, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_bl_place, 0, 1, web_post_tr(),
                   default_thumb_bl_place, 0, 1, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_br_place, -1, 0, web_post_tl(),
                   default_thumb_br_place, -1, 0, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_bl_place, -1, 0, web_post_tl(),
                   default_thumb_bl_place, -1, 0, web_post_bl())
    ])
    # thumb, corners
    shape = union([
        shape,
        wall_brace(default_thumb_br_place, -1, 0, web_post_bl(),
                   default_thumb_br_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
    ...
    shape = union([
        shape,
        wall_brace(default_thumb_mr_place, 0, -1, web_post_bl(),
                   default_thumb_br_place, 0, -1, web_post_br())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_ml_place, 0, 1, web_post_tl(),
                   default_thumb_bl_place, 0, 1, web_post_tr())
    ])
    shape = union([
        shape,
        wall_brace(default_thumb_bl_place, -1, 0, web_post_bl(),
                   default_thumb_br_place, -1, 0, web_post_tl())
    ])
    if default_1U_cluster:
        shape = union([
            shape,
            wall_brace(default_thumb_tr_place, 0, -1, web_post_br(),
                       (lambda sh: key_place(sh, 3, lastrow)), 0, -1,
                       web_post_bl())
        ])
    else:
        shape = union([
            shape,
            wall_brace(default_thumb_tr_place, 0, -1, thumb_post_br(),
                       (lambda sh: key_place(sh, 3, lastrow)), 0, -1,
                       web_post_bl())
        ])
...
def default_thumb_connection(side='right', skeleton=False):
    ...
    shape = union([
        shape,
        bottom_hull([
            left_key_place(translate(web_post(), wall_locate2(-1, 0)),
                           cornerrow,
                           -1,
                           low_corner=True,
                           side=side),
            left_key_place(translate(web_post(), wall_locate3(-1, 0)),
                           cornerrow,
                           -1,
                           low_corner=True,
                           side=side),
            default_thumb_ml_place(
                translate(web_post_tr(), wall_locate2(-0.3, 1))),
            default_thumb_ml_place(
                translate(web_post_tr(), wall_locate3(-0.3, 1))),
        ])
    ])
    ...
    shape = union([
        shape,
        hull_from_shapes([
            left_key_place(translate(web_post(), wall_locate1(-1, 0)),
                           cornerrow,
                           -1,
                           low_corner=True,
                           side=side),
    ...
    shape = union([
        ...
        hull_from_shapes([
            default_thumb_ml_place(web_post_tr()),
            default_thumb_ml_place(
                translate(web_post_tr(), wall_locate1(-0.3, 1))),
            default_thumb_ml_place(
                translate(web_post_tr(), wall_locate2(-0.3, 1))),
            default_thumb_ml_place(
                translate(web_post_tr(), wall_locate3(-0.3, 1))),
...
def tbcj_thumb_walls(skeleton=False):
    shape = union([
        wall_brace(tbcj_thumb_ml_place, -0.3, 1, web_post_tr(),
                   tbcj_thumb_ml_place, 0, 1, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(tbcj_thumb_bl_place, 0, 1, web_post_tr(),
                   tbcj_thumb_bl_place, 0, 1, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(tbcj_thumb_bl_place, -1, 0, web_post_tl(),
    ...
    shape = union([
        shape,
        wall_brace(tbcj_thumb_ml_place, 0, 1, web_post_tl(),
                   tbcj_thumb_bl_place, 0, 1, web_post_tr())
    ])
...
def mini_thumb_walls(skeleton=False):
    # thumb, walls
    shape = union([
        wall_brace(mini_thumb_mr_place, 0, -1, web_post_br(),
                   mini_thumb_tr_place, 0, -1, mini_thumb_post_br())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_mr_place, 0, -1, web_post_br(),
                   mini_thumb_mr_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_br_place, 0, -1, web_post_br(),
                   mini_thumb_br_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_bl_place, 0, 1, web_post_tr(),
                   mini_thumb_bl_place, 0, 1, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_br_place, -1, 0, web_post_tl(),
                   mini_thumb_br_place, -1, 0, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_bl_place, -1, 0, web_post_tl(),
                   mini_thumb_bl_place, -1, 0, web_post_bl())
    ])
    # thumb, corners
    shape = union([
        shape,
        wall_brace(mini_thumb_br_place, -1, 0, web_post_bl(),
                   mini_thumb_br_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
    ...
    shape = union([
        shape,
        wall_brace(mini_thumb_mr_place, 0, -1, web_post_bl(),
                   mini_thumb_br_place, 0, -1, web_post_br())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_bl_place, -1, 0, web_post_bl(),
                   mini_thumb_br_place, -1, 0, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(mini_thumb_tr_place, 0, -1, mini_thumb_post_br(),
                   (lambda sh: key_place(sh, 3, lastrow)), 0, -1,
                   web_post_bl())
    ])
...
def mini_thumb_connection(side='right', skeleton=False):
    ...
    shape = union([
        bottom_hull([
            ...
            left_key_place(translate(web_post(), wall_locate3(-1, 0)),
                           ...
                           low_corner=True,
                           side=side),
            mini_thumb_bl_place(translate(web_post_tr(), wall_locate2(-0.3,
                                                                      1))),
            mini_thumb_bl_place(translate(web_post_tr(), wall_locate3(-0.3,
                                                                      1))),
        ])
    ])
    ...
    shape = union([
        ...
        hull_from_shapes([
            mini_thumb_bl_place(web_post_tr()),
            mini_thumb_bl_place(translate(web_post_tr(), wall_locate1(-0.3,
                                                                      1))),
            mini_thumb_bl_place(translate(web_post_tr(), wall_locate2(-0.3,
                                                                      1))),
            mini_thumb_bl_place(translate(web_post_tr(), wall_locate3(-0.3,
...
def minidox_thumb_walls(skeleton=False):
    ...
    # thumb, walls
    shape = union([
        wall_brace(minidox_thumb_tr_place, 0, -1, minidox_thumb_post_br(),
                   minidox_thumb_tr_place, 0, -1, minidox_thumb_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(minidox_thumb_tr_place, 0, -1, minidox_thumb_post_bl(),
                   minidox_thumb_tl_place, 0, -1, minidox_thumb_post_br())
    ])
    shape = union([
        shape,
        wall_brace(minidox_thumb_tl_place, 0, -1, minidox_thumb_post_br(),
                   minidox_thumb_tl_place, 0, -1, minidox_thumb_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(minidox_thumb_tl_place, 0, -1, minidox_thumb_post_bl(),
                   minidox_thumb_ml_place, -1, -1, minidox_thumb_post_br())
    ])
    shape = union([
        shape,
        wall_brace(minidox_thumb_ml_place, -1, -1, minidox_thumb_post_br(),
                   minidox_thumb_ml_place, 0, -1, minidox_thumb_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(minidox_thumb_ml_place, 0, -1, minidox_thumb_post_bl(),
                   minidox_thumb_ml_place, -1, 0, minidox_thumb_post_bl())
    ])
    # thumb, corners
    shape = union([
        shape,
        wall_brace(minidox_thumb_ml_place, -1, 0, minidox_thumb_post_bl(),
                   minidox_thumb_ml_place, -1, 0, minidox_thumb_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(minidox_thumb_ml_place, -1, 0, minidox_thumb_post_tl(),
                   minidox_thumb_ml_place, 0, 1, minidox_thumb_post_tl())
    ])
    # thumb, tweeners
    shape = union([
        shape,
        wall_brace(minidox_thumb_ml_place, 0, 1, minidox_thumb_post_tr(),
                   minidox_thumb_ml_place, 0, 1, minidox_thumb_post_tl())
    ])
    shape = union([
        shape,
...
def minidox_thumb_connection(side='right', skeleton=False):
    ...
    shape = union([
        bottom_hull([
            ...
            left_key_place(translate(web_post(), wall_locate3(-1, 0)),
                           ...
                           low_corner=True,
                           side=side),
            minidox_thumb_bl_place(
                translate(minidox_thumb_post_tr(), wall_locate2(-0.3, 1))),
            minidox_thumb_bl_place(
                translate(minidox_thumb_post_tr(), wall_locate3(-0.3, 1))),
        ])
    ])
    ...
    shape = union([
        ...
        hull_from_shapes([
            minidox_thumb_ml_place(minidox_thumb_post_tr()),
            minidox_thumb_ml_place(
                translate(minidox_thumb_post_tr(), wall_locate1(0, 1))),
            minidox_thumb_ml_place(
                translate(minidox_thumb_post_tr(), wall_locate2(0, 1))),
            minidox_thumb_ml_place(
                translate(minidox_thumb_post_tr(), wall_locate3(0, 1))),
            minidox_thumb_tl_place(minidox_thumb_post_tl()),
        ])
    ])
...
def carbonfet_thumb_walls(skeleton=False):
    # thumb, walls
    shape = union([
        wall_brace(carbonfet_thumb_mr_place, 0, -1, web_post_br(),
                   carbonfet_thumb_tr_place, 0, -1, web_post_br())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_mr_place, 0, -1, web_post_br(),
                   carbonfet_thumb_mr_place, 0, -1.15, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_br_place, 0, -1, web_post_br(),
                   carbonfet_thumb_br_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_bl_place, -.3, 1, thumb_post_tr(),
                   carbonfet_thumb_bl_place, 0, 1, thumb_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_br_place, -1, 0, web_post_tl(),
                   carbonfet_thumb_br_place, -1, 0, web_post_bl())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_bl_place, -1, 0, thumb_post_tl(),
                   carbonfet_thumb_bl_place, -1, 0, web_post_bl())
    ])
    # thumb, corners
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_br_place, -1, 0, web_post_bl(),
                   carbonfet_thumb_br_place, 0, -1, web_post_bl())
    ])
    shape = union([
        shape,
    ...
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_mr_place, 0, -1.15, web_post_bl(),
                   carbonfet_thumb_br_place, 0, -1, web_post_br())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_bl_place, -1, 0, web_post_bl(),
                   carbonfet_thumb_br_place, -1, 0, web_post_tl())
    ])
    shape = union([
        shape,
        wall_brace(carbonfet_thumb_tr_place, 0, -1, web_post_br(),
                   (lambda sh: key_place(sh, 3, lastrow)), 0, -1,
                   web_post_bl())
    ])
    return shape
...
def carbonfet_thumb_connection(side='right', skeleton=False):
    ...
    shape = bottom_hull([
        ...
        left_key_place(translate(web_post(), wall_locate3(-1, 0)),
                       ...
                       side=side),
        carbonfet_thumb_bl_place(
            translate(thumb_post_tr(), wall_locate2(-0.3, 1))),
        carbonfet_thumb_bl_place(
            translate(thumb_post_tr(), wall_locate3(-0.3, 1))),
    ])
    ...
    shape = union([
        ...
        hull_from_shapes([
            ...
            carbonfet_thumb_bl_place(
                translate(thumb_post_tr(), wall_locate1(-0.3, 1))),
            carbonfet_thumb_bl_place(
                translate(thumb_post_tr(), wall_locate2(-0.3, 1))),
            carbonfet_thumb_bl_place(
                translate(thumb_post_tr(), wall_locate3(-0.3, 1))),
...
rj9_start = list(
    np.array([0, -3, 0]) + np.array(
        key_position(
            list(
                np.array(wall_locate3(0, 1)) +
...
external_start = list(
    # np.array([0, -3, 0])
    np.array([external_holder_width / 2, 0, 0]) + np.array(
        key_position(
            list(
                np.array(wall_locate3(0, 1)) +
                np.array([0, (mount_height / 2), 0])),
...
def external_mount_hole():
    ...
    undercut = box(external_holder_width + 8, 10.0,
                   external_holder_height + 8 + .1)
...
if oled_center_row is not None:
    base_pt1 = key_position(
        list(
            ...
            np.array([0, (mount_height / 2), 0])), 0, oled_center_row - 1)
    ...
    base_pt0 = key_position(
        list(
            np.array([-mount_width / 2, 0, 0]) +
            np.array([0, (mount_height / 2), 0])), 0, oled_center_row)

    oled_mount_location_xyz = (np.array(base_pt1) +
                               np.array(base_pt2)) / 2. + np.array(
                                   ((-left_wall_x_offset / 2), 0,
                                    0)) + np.array(oled_translation_offset)
    ...
    oled_mount_rotation_xyz = (rad2deg(angle_x), 0, -rad2deg(angle_z)
                               ) + np.array(oled_rotation_offset)
...
def tbiw_position_rotation():
    ...
    tbiw_mount_location_xyz = ((np.array(base_pt1) + np.array(base_pt2)) / 2. +
    ...
    angle_x = np.arctan2(base_pt1[2] - base_pt2[2], base_pt1[1] - base_pt2[1])
    angle_z = np.arctan2(base_pt1[0] - base_pt2[0], base_pt1[1] - base_pt2[1])
    tbiw_mount_rotation_xyz = (
        rad2deg(angle_x), 0, rad2deg(angle_z)) + np.array(tbiw_rotation_offset)
...
def oled_position_rotation(side='right'):
    ...
    if _oled_center_row is not None:
        base_pt1 = key_position(
            list(
                np.array([-mount_width / 2, 0, 0]) +
                np.array([0, (mount_height / 2), 0])), 0, _oled_center_row - 1)
        base_pt2 = key_position(
            list(
                np.array([-mount_width / 2, 0, 0]) +
                np.array([0, (mount_height / 2), 0])), 0, _oled_center_row + 1)
        base_pt0 = key_position(
            list(
                np.array([-mount_width / 2, 0, 0]) +
                np.array([0, (mount_height / 2), 0])), 0, _oled_center_row)
        ...
        oled_mount_location_xyz = (
            np.array(base_pt1) + np.array(base_pt2)) / 2. + np.array(
                ((-_left_wall_x_offset / 2), 0,
                 0)) + np.array(_oled_translation_offset)
        oled_mount_location_xyz[2] = (oled_mount_location_xyz[2] +
                                      base_pt0[2]) / 2

        angle_x = np.arctan2(base_pt1[2] - base_pt2[2],
                             base_pt1[1] - base_pt2[1])
        angle_z = np.arctan2(base_pt1[0] - base_pt2[0],
                             base_pt1[1] - base_pt2[1])
        if trackball_in_wall and (side == ball_side or ball_side == 'both'):
            ...
            oled_mount_rotation_xyz = (0, rad2deg(angle_x),
...
def oled_sliding_mount_frame(side='right'):
    ...
    mount_ext_height = (oled_mount_height + 2 * oled_edge_overlap_end +
                        oled_edge_overlap_connector +
                        oled_edge_overlap_clearance + 2 * oled_mount_rim)
    ...
    hole = box(mount_ext_width, mount_ext_up_height,
               oled_mount_cut_depth + .01)
    hole = translate(hole, (0., top_hole_start + top_hole_length / 2, 0.))

    hole_down = box(mount_ext_width, mount_ext_height,
    ...
    conn_hole_length = (oled_edge_overlap_end + oled_edge_overlap_connector +
    ...
    conn_hole = translate(conn_hole,
                          (0, conn_hole_start + conn_hole_length / 2,
    ...
    end_hole = translate(end_hole, (0, end_hole_start + end_hole_length / 2,
                                    -oled_edge_overlap_thickness))

    top_hole_start = -mount_ext_height / 2.0 + oled_mount_rim + oled_edge_overlap_end + oled_edge_overlap_connector
    top_hole_length = oled_mount_height
    top_hole = box(
        oled_mount_width, top_hole_length,
        oled_edge_overlap_thickness + oled_thickness - oled_edge_chamfer)
    top_hole = translate(top_hole,
                         (0, top_hole_start + top_hole_length / 2,
                          (oled_mount_depth - oled_edge_overlap_thickness -
                           oled_thickness - oled_edge_chamfer) / 2.0))

    top_chamfer_1 = box(oled_mount_width, top_hole_length, 0.01)
    top_chamfer_2 = box(oled_mount_width + 2 * oled_edge_chamfer,
                        top_hole_length + 2 * oled_edge_chamfer, 0.01)
...
def oled_clip_mount_frame(side='right'):
    ...
    mount_ext_height = (oled_mount_height + 2 * oled_clip_thickness +
                        2 * oled_clip_undercut + 2 * oled_clip_overhang +
                        2 * oled_mount_rim)
    ...
    shape = difference(
        shape,
        [box(oled_mount_width, oled_mount_height, oled_mount_depth + .1)])
    ...
    clip_undercut = box(
        oled_clip_width + 2 * oled_clip_width_clearance,
        oled_mount_height + 2 * oled_clip_thickness + 2 * oled_clip_overhang +
        2 * oled_clip_undercut, oled_mount_depth + .1)

    clip_undercut = translate(clip_undercut,
                              (0., 0., oled_clip_undercut_thickness))
    shape = difference(shape, [clip_undercut])

    plate = box(oled_mount_width + .1,
...
def oled_clip():
    mount_ext_width = oled_mount_width + 2 * oled_mount_rim
    mount_ext_height = (oled_mount_height + 2 * oled_clip_thickness +
                        2 * oled_clip_overhang + 2 * oled_clip_undercut +
                        2 * oled_mount_rim)

    oled_leg_depth = oled_mount_depth + oled_clip_z_gap

    shape = box(mount_ext_width - .1, mount_ext_height - .1,
                oled_mount_bezel_thickness)
    shape = translate(shape, (0., 0., oled_mount_bezel_thickness / 2.))

    hole_1 = box(oled_screen_width + 2 * oled_mount_bezel_chamfer,
                 oled_screen_length + 2 * oled_mount_bezel_chamfer, .01)
    hole_2 = box(oled_screen_width, oled_screen_length,
                 2.05 * oled_mount_bezel_thickness)
    hole = hull_from_shapes([hole_1, hole_2])

    shape = difference(shape,
                       [translate(hole, (0., 0., oled_mount_bezel_thickness))])
    ...
    latch_1 = box(oled_clip_width, oled_clip_overhang + oled_clip_thickness,
                  .01)
    latch_2 = box(oled_clip_width, oled_clip_thickness / 2,
                  oled_clip_extension)
    latch_2 = translate(
        latch_2,
        (0.,
         -(-oled_clip_thickness / 2 + oled_clip_thickness + oled_clip_overhang)
         / 2, -oled_clip_extension / 2))
    latch = hull_from_shapes([latch_1, latch_2])
    latch = translate(latch, (0., oled_clip_overhang / 2, -oled_leg_depth))
...
def oled_undercut_mount_frame(side='right'):
    ...
    shape = difference(
        shape,
        [box(oled_mount_width, oled_mount_height, oled_mount_depth + .1)])
    undercut = box(oled_mount_width + 2 * oled_mount_undercut,
...
def teensy_holder():
    ...
    teensy_holder_top_offset = (teensy_holder_top_length /
                                2) - teensy_holder_length
    ...
    s4 = translate(s4, [
        teensy_pcb_thickness + 5, teensy_holder_top_offset, 1 +
        (teensy_width / 2)
    ])
...
def screw_insert_shape(bottom_radius, top_radius, height):
    ...
    if bottom_radius == top_radius:
        ...
        base = translate(cone(r1=bottom_radius, r2=top_radius, height=height),
                         (0, 0, -height / 2))
...
def screw_insert(column, row, bottom_radius, top_radius, height, side='right'):
    ...
    if screws_offset == 'INSIDE':
        ...
        shift_left_adjust = wall_base_x_thickness * 2.5
        shift_right_adjust = -wall_base_x_thickness * 1.1
        shift_down_adjust = -wall_base_y_thickness * 2
        shift_up_adjust = -wall_base_y_thickness * 1.5
...
def thumb_screw_insert(bottom_radius,
                       top_radius,
                       height,
                       offset=None,
                       side='right'):
    ...
    if ('TRACKBALL'
            in thumb_style) and not (side == ball_side or ball_side == 'both'):
    ...
    if _thumb_style == 'MINI':
        ...
        xypositions[0][1] = xypositions[0][1] - .4 * (minidox_Usize -
                                                      1) * sa_length
    ...
    for xyposition in xypositions:
        ...
        shapes.append(
            translate(shape, [position[0], position[1], height / 2 + offset]))

    return shapes


def screw_insert_all_shapes(bottom_radius,
                            top_radius,
                            height,
                            offset=0,
                            side='right'):
    ...
    shape = (
        ...
        translate(
            screw_insert(0,
                         ...
                         top_radius,
                         height,
                         side=side), (0, left_wall_lower_y_offset, offset)),
        translate(
            screw_insert(3,
                         lastrow,
                         bottom_radius,
...
def thumb_screw_insert_holes(side='right'):
    return thumb_screw_insert(screw_insert_bottom_radius,
                              screw_insert_top_radius,
                              screw_insert_height + .02,
                              offset=-.01,
                              side=side)
...
def thumb_screw_insert_outers(offset=0.0, side='right'):
    ...
    return thumb_screw_insert(bottom_radius,
                              top_radius,
                              height,
                              offset=offset,
                              side=side)
...
def screw_insert_holes(side='right'):
    return screw_insert_all_shapes(screw_insert_bottom_radius,
                                   screw_insert_top_radius,
                                   screw_insert_height + .02,
                                   offset=-.01,
                                   side=side)
...
def screw_insert_outers(offset=0.0, side='right'):
    ...
    top_radius = screw_insert_outer_radius
    height = screw_insert_height + 1.5
    return screw_insert_all_shapes(bottom_radius,
                                   top_radius,
                                   height,
                                   offset=offset,
                                   side=side)
...
def wire_posts():
    ...
    shape = union([
        shape,
        default_thumb_ml_place(wire_post(-1, 6).translate([0, 0, -2.5]))
    ])
    shape = union(
        [shape,
         default_thumb_ml_place(wire_post(1, 0).translate([5, 0, -2]))])
...
def model_side(side="right"):
    ...
    if debug_exports:
        export_file(shape=shape,
                    fname=path.join(r"..", "things", r"debug_key_plates"))
    connector_shape = connectors()
    shape = union([shape, connector_shape])
    if debug_exports:
        export_file(shape=shape,
                    fname=path.join(r"..", "things", r"debug_connector_shape"))
    ...
    if controller_mount_type in [
            'RJ9_USB_TEENSY', 'RJ9_USB_WALL', 'USB_WALL', 'USB_TEENSY'
    ]:
    ...
    if trackball_in_wall and (side == ball_side
                              or ball_side == 'both') and separable_thumb:
    ...
    if debug_exports:
        export_file(shape=thumb_shape,
                    fname=path.join(r"..", "things", r"debug_thumb_shape"))
    thumb_connector_shape = thumb_connectors(side=side)
    if debug_exports:
        export_file(shape=thumb_connector_shape,
                    fname=path.join(r"..", "things",
                                    r"debug_thumb_connector_shape"))

    thumb_wall_shape = thumb_walls(side=side, skeleton=skeletal)
    thumb_wall_shape = union(
        [thumb_wall_shape, *thumb_screw_insert_outers(side=side)])
    thumb_connection_shape = thumb_connection(side=side, skeleton=skeletal)

    if debug_exports:
        thumb_test = union([
            thumb_shape, thumb_connector_shape, thumb_wall_shape,
            thumb_connection_shape
        ])
        export_file(shape=thumb_test,
                    fname=path.join(r"..", "things",
                                    r"debug_thumb_test_{}_shape".format(side)))

    thumb_section = union([
        thumb_shape, thumb_connector_shape, thumb_wall_shape,
        thumb_connection_shape
    ])
    thumb_section = difference(thumb_section,
                               [union(thumb_screw_insert_holes(side=side))])

    has_trackball = False
    if ('TRACKBALL' in thumb_style) and (side == ball_side
                                         or ball_side == 'both'):
    ...
    if plate_pcb_clear:
        thumb_section = difference(thumb_section,
                                   [thumb_pcb_plate_cutouts(side=side)])
    ...
    if separable_thumb:
        ...
        if trackball_in_wall and (side == ball_side or ball_side
                                  == 'both') and not separable_thumb:
            tbprecut, tb, tbcutout, sensor, ball = generate_trackball_in_wall()
...
def baseplate(wedge_angle=None, side='right'):
    if ENGINE == 'cadquery':
        ...
        thumb_section = difference(
            thumb_section, [union(thumb_screw_insert_holes(side=side))])

        shape = union([
            case_walls(side=side), *screw_insert_outers(side=side),
            thumb_section
        ])
        tool = screw_insert_all_shapes(screw_hole_diameter / 2.,
        ...
        tool = thumb_screw_insert(screw_hole_diameter / 2.,
                                  screw_hole_diameter / 2.,
                                  350,
                                  side=side)
        for item in tool:
        ...
        outside = shape.vertices(
            cq.DirectionMinMaxSelector(cq.Vector(1, 0, 0), True)).objects[0]
        sizes = []
        max_val = 0
        inner_index = 0
        ...
        if wedge_angle is not None:
            cq.Workplane('XY').add(
                cq.Solid.revolve(outerWire, innerWires, angleDegrees,
                                 axisStart, axisEnd))
        else:
            inner_shape = cq.Workplane('XY').add(
                cq.Solid.extrudeLinear(outerWire=inner_wire,
                                       innerWires=[],
                                       vecNormal=cq.Vector(
                                           0, 0, base_thickness)))
            ...
            shape = cq.Workplane('XY').add(
                cq.Solid.extrudeLinear(outer_wire, cutout,
                                       cq.Vector(0, 0, base_rim_thickness)))
            hole_shapes = []
            for hole in holes:
                loc = hole.Center()
                hole_shapes.append(
                    translate(
                        cylinder(screw_cbore_diameter / 2.0,
                                 screw_cbore_depth), (loc.x, loc.y, 0)
        ...
        tool = translate(union(screw_insert_screw_holes(side=side)),
                         [0, 0, -10])
...
def run():
    ...
    export_file(shape=mod_r,
                fname=path.join(save_path, config_name + r"_right"))
    export_file(shape=tmb_r,
                fname=path.join(save_path, config_name + r"_thumb_right"))

    #base = baseplate(mod_r, tmb_r, side='right')
    base = baseplate(side='right')
    export_file(shape=base,
                fname=path.join(save_path, config_name + r"_right_plate"))
    export_dxf(shape=base,
               fname=path.join(save_path, config_name + r"_right_plate"))

    if symmetry == "asymmetric":
        mod_l, tmb_l = model_side(side="left")
        export_file(shape=mod_l,
                    fname=path.join(save_path, config_name + r"_left"))
        export_file(shape=tmb_l,
                    fname=path.join(save_path, config_name + r"_thumb_left"))

        #base_l = mirror(baseplate(mod_l, tmb_l, side='left'), 'YZ')
        base_l = mirror(baseplate(side='left'), 'YZ')
        export_file(shape=base_l,
                    fname=path.join(save_path, config_name + r"_left_plate"))
        export_dxf(shape=base_l,
                   fname=path.join(save_path, config_name + r"_left_plate"))

    else:
        export_file(shape=mirror(mod_r, 'YZ'),
                    fname=path.join(save_path, config_name + r"_left"))

        lbase = mirror(base, 'YZ')
        export_file(shape=lbase,
                    fname=path.join(save_path, config_name + r"_left_plate"))
        export_dxf(shape=lbase,
                   fname=path.join(save_path, config_name + r"_left_plate"))

    if oled_mount_type == 'UNDERCUT':
        export_file(shape=oled_undercut_mount_frame()[1],
                    fname=path.join(save_path,
                                    config_name + r"_oled_undercut_test"))

    if oled_mount_type == 'SLIDING':
        export_file(shape=oled_sliding_mount_frame()[1],
                    fname=path.join(save_path,
                                    config_name + r"_oled_sliding_test"))

    if oled_mount_type == 'CLIP':
        oled_mount_location_xyz = (0.0, 0.0, -oled_mount_depth / 2)
        oled_mount_rotation_xyz = (0.0, 0.0, 0.0)
        export_file(shape=oled_clip(),
                    fname=path.join(save_path, config_name + r"_oled_clip"))
        export_file(shape=oled_clip_mount_frame()[1],
                    fname=path.join(save_path,
                                    config_name + r"_oled_clip_test"))
        export_file(shape=union((oled_clip_mount_frame()[1], oled_clip())),
                    fname=path.join(save_path,
                                    config_name + r"_oled_clip_assy_test"))
